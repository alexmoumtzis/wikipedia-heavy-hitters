...
object ReservoirSameMemory {
  ...
  def main(args: Array[String]): Unit = {
    ...
    println(s"[Reservoir-sweep] trueHH=${trueHHKeys.size}  trueN=$trueN")
    ...
    val rows = DEFAULT_MEMORY_TIERS_KB.map { memKB =>
      ...
      println(s"\n[Reservoir] memKB=$memKB  M=$M  seeds=${DEFAULT_SEEDS.mkString("|")}")
    ...
    writePerAlgorithmCsv(outputDir, "Reservoir", rows)
    writeComparison(outputDir, rows)
    spark.stop()
    println("[Reservoir-sweep] done.")
  }
}
