...
object ReservoirSkew extends SkewBenchmarkBase[Seq[ReservoirSampling]] {
  ...
  def algoName = "Reservoir"
  def needsKeyUniverse = false

  def buildStructure(memKB: Int): Seq[ReservoirSampling] = {
    val bBytes = memKB.toLong * 1024L
    val M      = math.max(1L, bBytes / AVG_BYTES_PER_SLOT).toInt
    SEEDS.map(seed => new ReservoirSampling(M, seed))
  }
  def update(s: Seq[ReservoirSampling], key: String, views: Long): Unit = s.foreach(_.update(key, views))
  def totalWeight(s: Seq[ReservoirSampling]): Long = s.head.totalWeight
  def candidates(s: Seq[ReservoirSampling], uniqueKeys: Array[String]): Iterator[(String, Long)] = {
