...
object SpaceSavingSameThreshold {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val spark = SparkSession.builder()
      .appName("SpaceSavingSameThreshold")
      .master("local[*]")
    ...
    println(s"[SS-same-threshold] targetPhi=$PHI trueHH=${trueHHKeys.size} trueN=$trueN")
    ...
    val rows = DEFAULT_MEMORY_TIERS_KB.map { memKB =>
      ...
      println(s"\n[SS-ST] memKB=$memKB  capacity=$capacity  eps=${"%.2e".format(eps)}")
    ...
    writeThresholdCsv(outputDir, "SS", rows)
    writeThresholdComparison(outputDir, rows)
    spark.stop()
    println("[SS-same-threshold] done.")
  }
